 ...
 * decorative WS2812 status strip, AND bridges the host UART to the UART backplane
 * (the wheels leaf, NODE 0x05). ALL the mechanical wiring (the root route table,
 * hub_on_body relay, the sense ticks, the LED command dispatch, the schedule,
 * hub_setup/hub_tasks) is GENERATED into firmware/gen/segby_v1/blaster.glue.h
 * from the IR. This file supplies only the device-specific hooks:
 *
 *   * blaster_device_setup()       — strip dark at boot (peripherals lazy-init).
 ...
 * decorative LED has no floor, §09).
 *
 * Including blaster.glue.h pulls in the generated hub_setup/hub_on_body/hub_tasks
...
/* --- device: strip dark at boot; peripherals are lazy-init'd on first read. The
 * floors n/a here (the Blaster has no actuator floor — the LED is decorative). */
extern "C" void blaster_device_setup(void) {
  Led off = {0, 0, 0};
  blaster_status_led_drive(&off); /* strip dark at boot (no floor; just a clean start) */
}

/* --- device: the REAL MPU-9250 read on Wire(SDA 21 / SCL 22) @ addr 0x68, ported
 * from climber's Mpu9250Backend.cpp (deployed on this rig). Lazy one-time
 * bring-up (open the bus, WHO_AM_I sanity-check, wake + PLL clock); a failed init
 * retries next tick. The 14-byte burst from ACCEL_XOUT_H keeps the sample
 ...
 * orientation — it packs an IDENTITY quaternion (qw=1) and the host recovers pitch
 * from accel+gyro (complementary filter). On a bounded-read failure it returns
 * false so pose's seq stalls and the reader goes stale (legible). --- */
extern "C" bool blaster_pose_read(Imu *out) {
  ...
  /* Identity orientation — the MCU does not fuse; the host's complementary filter
   * recovers pitch from accel+gyro (LOCKED design, §09). */
  out->qw = 1.0f;
  out->qx = 0.0f;
  out->qy = 0.0f;
  out->qz = 0.0f;
