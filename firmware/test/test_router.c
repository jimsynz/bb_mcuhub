...
static void on_local(const Frame *f, void *ctx) { (void)ctx; g_local++; g_last = *f; }
static void on_up(const Frame *f, void *ctx) { (void)ctx; g_up++; g_last = *f; }
static void on_down(const Frame *f, void *ctx) { (void)ctx; g_down++; g_last = *f; }
...
static Frame make(uint8_t node, uint8_t port, uint16_t seq, uint64_t t) {
  ...
  f.port = port;
  f.seq = seq;
  f.stamped = true; /* the router is meaning-blind; it forwards the struct as-is */
  f.t_dev = t;
  f.payload[0] = 0xAB;
  f.payload[1] = 0xCD;
  f.payload_len = 2;
...
int main(void) {
  ...
  memset(&r, 0, sizeof(r));
  r.my_node = 0x02;
  for (int i = 0; i < 256; i++) r.route_table[i] = LINK_UP;
  ...
  CHECK(g_up == 1 && g_local == 0 && g_down == 0, "frame for a non-child node → forward_up");

  /* a leaf hub: everything is local, nothing forwards */
  Router leaf;
  memset(&leaf, 0, sizeof(leaf));
  leaf.my_node = 0x05;
  for (int i = 0; i < 256; i++) leaf.route_table[i] = LINK_LOCAL;
  RouterSinks leaf_sinks = {on_local, on_up, on_down, NULL};

  reset();
  Frame f_leaf = make(0x05, 0x28, 3, 9);
  router_route(&leaf, &f_leaf, &leaf_sinks);
  CHECK(g_local == 1 && g_up == 0 && g_down == 0, "leaf: a command for it is delivered local");
