 ...
 * plain UART (BACKPLANE_TRANSPORT_UART == 1) the root hub ↔ children link
 * carries the SAME COBS+CRC frame as the host↔root seam, with NO CAN
 * segmentation: COBS already frames arbitrary-length bodies, so a wide body
 * (the 54-byte IMU) rides one frame. This harness exercises the exact path
 * link_esp32.cpp's UART backplane uses — transport_encode on TX,
 * transport_decoder_feed on RX (transport.c) — and proves a wide body
 * round-trips byte-identical without fragmentation. Pure logic, no hardware. */
#include "crc16.h"
#include "transport.h"
#include <stdio.h>
#include <string.h>
...
static size_t make_imu_body(uint8_t *out, uint16_t seq) {
  ...
  for (int i = 0; i < 8; i++)
    out[n++] = (uint8_t)(0xA0 + i); /* T_DEV (stamped) */
  for (int i = 0; i < 42; i++)
...
int main(void) {
   ...
   * TX path the UART backplane uses), fed back through a TransportDecoder,
   * round- trips byte-identical in ONE frame — no segmentation. */
  ...
  CHECK(wire[w - 1] == 0x00,
        "the wire frame ends in the single 0x00 COBS delimiter");
  ...
  for (size_t i = 0; i < w; i++)
    if (wire[i] == 0x00)
      n_delims++;
  CHECK(n_delims == 1,
        "the wide body rides ONE COBS frame (no CAN-style fragmentation)");
  ...
  /* feeding byte-at-a-time (as link_pump does over Serial2) delivers
   * identically */
  {
    TransportDecoder dd;
    transport_decoder_init(&dd);
    reset_capture();
    for (size_t i = 0; i < w; i++)
      transport_decoder_feed(&dd, &wire[i], 1, on_body, NULL);
    CHECK(
        g_delivered == 1 && g_body_len == blen &&
            memcmp(g_body, body, blen) == 0,
  ...
  {
    uint8_t b1[FRAME_MAX_BODY];
    size_t l1 = make_imu_body(b1, 100);
    uint8_t b2[FRAME_MAX_BODY];
    size_t l2 = make_imu_body(b2, 101);
    uint8_t w1[FRAME_MAX_WIRE];
    size_t n1 = transport_encode(b1, l1, w1, sizeof(w1));
    uint8_t w2[FRAME_MAX_WIRE];
    size_t n2 = transport_encode(b2, l2, w2, sizeof(w2));
    ...
    got_1 =
        (g_delivered == 1 && g_body_len == l1 && memcmp(g_body, b1, l1) == 0);
    reset_capture();
    transport_decoder_feed(&dd, w2, n2, on_body, NULL);
    got_2 =
        (g_delivered == 1 && g_body_len == l2 && memcmp(g_body, b2, l2) == 0);
    CHECK(got_1 && got_2,
          "two back-to-back framed bodies both deliver, in order");
  }

  /* fail-closed: a one-bit flip in a framed body fails the CRC and is dropped,
   * never delivered partial — the same guarantee the host↔root seam gives
   * (§04). */
  {
    uint8_t b[FRAME_MAX_BODY];
    size_t l = make_imu_body(b, 202);
    uint8_t wb[FRAME_MAX_WIRE];
    size_t nb = transport_encode(b, l, wb, sizeof(wb));
