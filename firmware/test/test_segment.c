...
static size_t make_imu_body(uint8_t *out, uint16_t seq) {
  ...
  for (int i = 0; i < 8; i++)
    out[n++] = (uint8_t)(0xA0 + i); /* T_DEV */
  for (int i = 0; i < 40; i++)
    out[n++] = (uint8_t)(i * 7 + 1); /* payload */
  return n;                          /* 52 */
}
...
int main(void) {
  ...
  for (size_t i = 0; i < nf; i++)
    seg_reasm_feed(&r, &frames[i], on_body, NULL);
   ...
   */
  {
    ...
    /* the (NODE,PORT) hardware-filter view is the top 16 bits,
     * fragment-agnostic */
    ...
    CHECK(estop < data_frame,
          "NODE 0x00 (e-stop) still wins arbitration over any data frame");
  }

  /* a single-frame-sized body (an 8-byte effort command → 10 with CRC → 2
   * frags) still rides FIRST+LAST framing and reassembles */
  {
    uint8_t cmd[8] = {0x02, 0x28, 0x00, 0x07, 0x3F, 0x80, 0x00, 0x00};
    CanFrame fr[SEG_MAX_FRAGS];
    size_t n = 0;
    bool sok =
        seg_split(0x02, 0x28, 7, cmd, sizeof(cmd), fr, SEG_MAX_FRAGS, &n);
   ...
   * --- */
  {
    uint8_t b1[SEG_MAX_BODY];
    size_t l1 = make_imu_body(b1, 100);
    uint8_t b2[SEG_MAX_BODY];
    size_t l2 = make_imu_body(b2, 101);
    CanFrame f1[SEG_MAX_FRAGS], f2[SEG_MAX_FRAGS];
    size_t n1 = 0, n2 = 0;
    ...
    /* skip f1[3] */
    seg_reasm_feed(&rr, &f1[4], on_body,
                   NULL); /* idx 4 != expected 3 → abandon */
    CHECK(g_delivered == 0,
          "lost fragment: the gapped body is NEVER delivered");
    CHECK(rr.rx_frag_drop == 1,
          "lost fragment: the partial is dropped + counted");

    /* now body 101 arrives whole → re-seeds and delivers cleanly */
    for (size_t i = 0; i < n2; i++)
      seg_reasm_feed(&rr, &f2[i], on_body, NULL);
    CHECK(g_delivered == 1 && g_body_len == l2 && memcmp(g_body, b2, l2) == 0,
          "the next FIRST re-seeds: body 101 delivers byte-identical after the "
          "loss");
  }

  /* --- fail-closed: an orphan non-first fragment never seeds a body --- */
  {
    uint8_t b[SEG_MAX_BODY];
    size_t l = make_imu_body(b, 200);
    CanFrame f[SEG_MAX_FRAGS];
    size_t n = 0;
    ...
    CHECK(rr.rx_frag_orphan == 1,
          "orphan fragment: counted as rx_frag_orphan, not seeding a body");
    /* and a real body afterward still works — the orphan left no residue */
    for (size_t i = 0; i < n; i++)
      seg_reasm_feed(&rr, &f[i], on_body, NULL);
  ...
  /* --- fail-closed: a reordered fragment abandons the partial --- */
  {
    ...
    seg_reasm_feed(&rr, &f[2], on_body,
                   NULL); /* out of order: expected 1, got 2 */
    CHECK(
        g_delivered == 0 && rr.rx_frag_drop == 1,
  ...
  {
    uint8_t b[SEG_MAX_BODY];
    size_t l = make_imu_body(b, 202);
    CanFrame f[SEG_MAX_FRAGS];
    size_t n = 0;
    seg_split(0x02, 0xEF, 202, b, l, f, SEG_MAX_FRAGS, &n);
    f[3].data[1] ^=
        0x40; /* corrupt a payload byte mid-body (a re-frame bit-flip) */
    ...
    for (size_t i = 0; i < n; i++)
      seg_reasm_feed(&rr, &f[i], on_body, NULL);
    CHECK(g_delivered == 0,
          "CRC corruption: a corrupted body is NEVER delivered");
    CHECK(rr.rx_crc_fail == 1,
          "CRC corruption: counted as rx_crc_fail (caught after reassembly)");
  }
   ...
   * --- */
  {
     ...
     * bodies with DIFFERENT seqlo: a stale frag from body A splicing into body
     * B. */
    uint8_t bA[SEG_MAX_BODY];
    size_t lA = make_imu_body(bA, 10); /* seqlo 10 */
    uint8_t bB[SEG_MAX_BODY];
    size_t lB = make_imu_body(bB, 11); /* seqlo 11 */
    CanFrame fA[SEG_MAX_FRAGS], fB[SEG_MAX_FRAGS];
    size_t nA = 0, nB = 0;
    ...
    seg_reasm_feed(&rr, &fA[1], on_body,
                   NULL); /* idx 1 ok, but seqlo 10 != 11 */
    CHECK(g_delivered == 0 && rr.rx_frag_drop == 1,
          "seqlo alias: a fragment from a different body is rejected, not "
          "spliced");
  }
   ...
   */
  {
    ...
    CHECK(ok510 && n == 64, "ceiling: a 510-byte body (→512 with CRC) fits in "
                            "exactly 64 fragments");
    SegReasm rr;
    seg_reasm_init(&rr);
    reset_capture();
    for (size_t i = 0; i < n; i++)
      seg_reasm_feed(&rr, &f[i], on_body, NULL);
    CHECK(g_delivered == 1 && g_body_len == 510 &&
              memcmp(g_body, big, 510) == 0,
    ...
    CHECK(!ok511 && n == 0,
          "ceiling: a 511-byte body is REFUSED (over 512), not truncated");
  }
   ...
   * Each (node,port) has its own buffer, so interleaving must not corrupt
   * either. */
  {
    uint8_t ba[SEG_MAX_BODY];
    size_t la = make_imu_body(ba, 300);
    ba[0] = 0x05;
    uint8_t bb[SEG_MAX_BODY];
    size_t lb = make_imu_body(bb, 301);
    bb[0] = 0x06;
    CanFrame fa[SEG_MAX_FRAGS], fb[SEG_MAX_FRAGS];
    size_t na = 0, nb = 0;
    ...
    for (size_t i = 0; i < na; i++) {
      ...
      if (g_delivered) {
        delivered_a += (g_body_len == la && memcmp(g_body, ba, la) == 0);
      }
      reset_capture();
      seg_reasm_feed(&rr, &fb[i], on_body, NULL);
      if (g_delivered) {
        delivered_b += (g_body_len == lb && memcmp(g_body, bb, lb) == 0);
      }
    }
    CHECK(delivered_a == 1 && delivered_b == 1,
          "interleaved streams: both (node,port) bodies reassemble "
          "independently, byte-identical");
  }

  /* --- structural reclaim: a new FIRST mid-stream re-seeds (the no-timeout
   * rule) --- The "no reassembly timeout in v1" decision rests on this: a
   * stalled partial is reclaimed by the next FIRST for that key, not by a wall
   * clock. */
  {
    uint8_t bA[SEG_MAX_BODY];
    size_t lA = make_imu_body(bA, 400);
    uint8_t bB[SEG_MAX_BODY];
    size_t lB = make_imu_body(bB, 401);
    CanFrame fA[SEG_MAX_FRAGS], fB[SEG_MAX_FRAGS];
    size_t nA = 0, nB = 0;
    ...
    seg_reasm_init(&rr);
    reset_capture();
    /* open body A and feed a few frags, then ABANDON it by sending body B's
     * FIRST (e.g. body A's tail was lost on the bus and never arrived) */
    ...
    CHECK(g_delivered == 1 && g_body_len == lB && memcmp(g_body, bB, lB) == 0,
          "structural reclaim: a mid-stream FIRST re-seeds; body B delivers, "
          "A's partial discarded");
    CHECK(rr.rx_crc_fail == 0,
          "structural reclaim: no false CRC failure from spliced bytes");
  }
