 ...
 * children), both carrying the SAME COBS+CRC frame. Mirrors the reference climber
 * transport but conforms to the new design: one frame, big-endian body, end-to-end
 * CRC across the re-framing boundary.
 *
 ...
 *                                     54-byte IMU) rides one COBS frame, exactly
 *                                     like the host↔root UART seam.
 *
 * Build-time role (UP direction):
 *   -DROOT_HUB  → UP is UART to the host; the backplane (DOWN) reaches children.
 *   (default)   → UP is the backplane to the parent (a leaf).
 *
...
#if BACKPLANE_TRANSPORT_UART
/* The UART backplane is a streaming COBS+CRC seam (transport.c), exactly like the
 * host↔root UART. No segmentation: a wide body rides one COBS frame. */
...
/* Legible failure counters (§03). On a UART backplane there is no fragmentation,
 * so the frag/oversize counters are vacuously zero; rx_drop is the seam's analog
 * of the CAN seam's drop (a corrupt/truncated COBS frame). */
...
#if defined(ROOT_HUB)
static TransportDecoder g_uart_rx; /* the host UART seam — only the root hub has one */
...
void link_begin(void) {
  ...
  Serial2.begin(BACKPLANE_UART_BAUD, SERIAL_8N1, BACKPLANE_UART_RX_PIN, BACKPLANE_UART_TX_PIN);
#else
 ...
 * deeper, it is the backplane. The body is identical; only the transport differs. */
...
#if defined(ROOT_HUB)
  /* UP from the root hub is the host UART (Serial) — unchanged by the backplane. */
  uint8_t wire[FRAME_MAX_WIRE];
  size_t w = transport_encode(body, body_len, wire, sizeof(wire));
  Serial.write(wire, w);
#elif BACKPLANE_TRANSPORT_UART
  /* UP from a leaf over a UART backplane: COBS+CRC the whole body into one frame.
   * No segmentation — a wide body rides one COBS frame, exactly like the host
   * seam (transport.c). */
  uint8_t wire[FRAME_MAX_WIRE];
  size_t w = transport_encode(body, body_len, wire, sizeof(wire));
  Serial2.write(wire, w);
#else
  /* UP from a leaf over a CAN backplane is segmented (§03): split body || CRC into
   * ordered frames, fragment metadata in the 13 reserved id bits. ESP32's built-in
   * TWAI is classic CAN (8-byte data field), so even a small body fragments (an
   * 8-byte body → 10 with CRC → 2 frames); a CAN-FD board would carry up to 512 B
   * with fewer frames. The whole body crosses byte-identical — never truncated. */
...
#if BACKPLANE_TRANSPORT_UART
  /* The UART backplane: feed bytes into the COBS+CRC decoder. It calls g_on_body
   * only with a complete, CRC-clean body (CRC stripped) — a corrupt/truncated
   * frame is dropped and counted, never delivered partial (§03/§04). */
