...
size_t transport_encode(const uint8_t *body, size_t body_len, uint8_t *out,
  ...
  if (body_len + 2 > sizeof(framed))
    return 0;

  memcpy(framed, body, body_len);
  uint16_t crc = crc16_ccitt_false(body, body_len);
  be_put_u16(&framed[body_len],
             crc); /* CRC is big-endian on the wire, matching Elixir */
...
static void handle_frame(TransportDecoder *d, transport_on_body_fn on_body,
                         void *ctx) {
  if (d->len == 0)
    return; /* empty run between two delimiters → ignore */
