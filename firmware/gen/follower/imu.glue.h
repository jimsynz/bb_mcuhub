/* GENERATED by BBMcuhub.Gen.WireGen — do not edit. imu.glue.h
   generated per-hub glue (router, floor, ticks) — §08 */
/* GENERATED firmware glue for the imu hub — ROOT hub (host UART ↔ backplane + local ports).
   ...
   device hooks in mcu/imu.{c,cpp} (prototypes in imu.device.h).
...
#include "imu.device.h" /* the hand-written device hooks */
}
...
/* Optional per-loop telemetry hook (§08). The glue calls imu_post_control() at
   the end of every control_tick. The device file can provide its own by doing
   `#define IMU_POST_CONTROL_OVERRIDE` BEFORE including this header (then
   implementing imu_post_control() itself); otherwise this no-op default is used. */
#ifndef IMU_POST_CONTROL_OVERRIDE
extern "C" void imu_post_control(void) {}
#endif
