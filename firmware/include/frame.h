 ...
 * BBMcuhub.Wire.FramingCOBS exactly, byte-for-byte (the parity vectors prove it).
 *
 * Body (the CRC-covered bytes):  NODE u8 · PORT u8 · SEQ u16 · T_DEV u64 · PAYLOAD
 * Wire frame:  COBS( body || CRC16(body) ) || 0x00
 *
...
/* Header is per-port (§04): the base NODE(1)+PORT(1)+SEQ(2) always present, plus
 * T_DEV(8) only on a STAMPED port. */
...
/* Parse a CRC-verified body into `f`. `stamped` says whether a t_dev follows the
 * base header — the caller knows this from the per-(node,port) contract, exactly
 * as the Elixir decoder learns it from PortIndex (§04). Returns true on success. */
