defmodule BBMcuhub.Test.VirtualHub do
  ...
  def handle_call({:tick, now_ms}, _from, st) do
    ports =
      Map.new(st.ports, fn {pid, p} ->
        {floor, _drive, armed} = VHubNif.floor_tick(p.floor, now_ms)
        p = %{p | floor: floor, armed: armed}
        emit_status(st.owner, p)
        {pid, p}
      end)

    {:reply, :ok, %{st | ports: ports, now_ms: now_ms}}
  end
  ...
  # Emit this port's status (applied_seq, floored?) up to the host, C-framed.
  defp emit_status(owner, p) do
  ...
  # Deliver a body to the host owner in the same message shape the UART transport uses.
  defp deliver(owner, body), do: Kernel.send(owner, {:circuits_uart, :vhub, body})
