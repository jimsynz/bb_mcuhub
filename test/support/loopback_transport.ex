defmodule BBMcuhub.Test.LoopbackTransport do
  @moduledoc """
  An in-process transport for tests (§07): it carries outbound bodies through the
  *real* `FramingCOBS` encode→bytes→decode path and back to the owner, so the host
  stack is exercised over the actual COBS+CRC seam without any hardware.
