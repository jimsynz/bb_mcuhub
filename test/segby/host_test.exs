defmodule BBMcuhub.Robots.SegbyV1.HostTest do
  @moduledoc """
  The segby_v1 host integration proof (§07/§09) — the WHOLE host stack stood up
  by the launcher (`BBMcuhub.Robots.SegbyV1.Host`) over the real COBS+CRC seam,
  with no hardware: the BeamBots tree (sensor + actuator views + the `:balance`
  controller + the `:teleop` command) plus the `LinkOwner` on a loopback
  transport.
  ...
  alias BBMcuhub.Robots.SegbyV1.Host
  alias BBMcuhub.Segby.Balance
  ...
  describe "teleop loop closed: a bb_tui-delivered intent biases the wire" do
    ...
    test "the controller's BB.Message Twist path updates last_teleop directly" do
      ...
      {:ok, ctrl} =
        BBMcuhub.Test.ViewHarness.start(Balance,
      ...
      view = BBMcuhub.Test.ViewHarness.view_state(ctrl)
