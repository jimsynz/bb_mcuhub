defmodule BBMcuhub.Robots.SegbyV1Test do
  @moduledoc """
  ...
  Beyond that, this asserts the IR `BBMcuhub.Robots.SegbyV1` projects (§06):
  the two-hub, dual-port-one-node, UART-backplane shape locked for segby_v1.

    * Blaster (NODE 0x02), the root comms hub: `pose` (out/imu, stamped),
      `range_front` (out/range), `status_led` (in/led, decorative — no floor).
